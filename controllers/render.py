...
@render.route("/pag_faixa")
def pag_faixa():
     return render_template("sensors/sensores.html")
...
@render.route("/update_dispositivo/<id>")
def update_dispositivo(id):
   
    dispositivo = db.session.query(Dispositivo,Sensor)\
                        .join(Dispositivo, Dispositivo.id == Sensor.id)\
                        .filter(Dispositivo.id == int(id)).first()
    
    return render_template("/device/update_device.html", disposito = dispositivo)
...
@render.route("/salvar_dispositivo_changes", methods=["POST"])
def salvar_dispositivo_changes():
    if request.method == "POST":
        data = request.form.copy()
        print(data)
        Dispositivo.update_dispositivo(data)
        return redirect(url_for('render.listar_dispositivos'))
    else:
